...
'	response.Write(NextChoice)
	...
	if ID="-1" then
		...
		colfield=" ( ID,LB,MC,FF,YYFW,LEIB,GJBZ,BZMC,BJZT,LRRY,LRRQ) "
		...
		colvalue=colvalue&",'"&request.Form("GJBZ")&"'"
		colvalue=colvalue&",'"&request.Form("BZMC")&"',1,'"&user&"','"&SYSDATE&"') "
...
'		response.Write(strSQL)
	...
	if NextChoice=1 then
		response.Redirect("tab_effect.asp?IsNew=1")
	end if
